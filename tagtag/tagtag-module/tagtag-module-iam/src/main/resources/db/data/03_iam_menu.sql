...
SELECT 0, 'IAM', 'iam', '/iam', NULL, 'lucide:users', 10, 1, 0, 0, 0, NULL, '身份与访问管理', CURRENT_TIMESTAMP
WHERE NOT EXISTS (SELECT 1 FROM iam_menu WHERE menu_code='iam');

INSERT INTO iam_menu (parent_id, menu_name, menu_code, path, component, icon, sort, status, menu_type, is_hidden, is_external, external_url, remark, create_time)
SELECT p.id, '用户管理', 'iam:user', '/iam/user', 'modules/iam/user/index', 'lucide:user', 10, 1, 1, 0, 0, NULL, '用户管理页面', CURRENT_TIMESTAMP
FROM iam_menu p WHERE p.menu_code='iam'
  AND NOT EXISTS (SELECT 1 FROM iam_menu WHERE menu_code='iam:user');

INSERT INTO iam_menu (parent_id, menu_name, menu_code, path, component, icon, sort, status, menu_type, is_hidden, is_external, external_url, remark, create_time)
SELECT p.id, '角色管理', 'iam:role', '/iam/role', 'modules/iam/role/index', 'lucide:shield-question', 20, 1, 1, 0, 0, NULL, '角色管理页面', CURRENT_TIMESTAMP
FROM iam_menu p WHERE p.menu_code='iam'
  AND NOT EXISTS (SELECT 1 FROM iam_menu WHERE menu_code='iam:role');

INSERT INTO iam_menu (parent_id, menu_name, menu_code, path, component, icon, sort, status, menu_type, is_hidden, is_external, external_url, remark, create_time)
SELECT p.id, '部门管理', 'iam:dept', '/iam/dept', 'modules/iam/dept/index', 'lucide:shield', 30, 1, 1, 0, 0, NULL, '部门管理页面', CURRENT_TIMESTAMP
FROM iam_menu p WHERE p.menu_code='iam'
  AND NOT EXISTS (SELECT 1 FROM iam_menu WHERE menu_code='iam:dept');

INSERT INTO iam_menu (parent_id, menu_name, menu_code, path, component, icon, sort, status, menu_type, is_hidden, is_external, external_url, remark, create_time)
SELECT p.id, '菜单管理', 'iam:menu', '/iam/menu', 'modules/iam/menu/index', 'lucide:menu', 40, 1, 1, 0, 0, NULL, '菜单管理页面', CURRENT_TIMESTAMP
FROM iam_menu p WHERE p.menu_code='iam'
