...
**Core Components**:
- `utils/`: Collection of utility classes (BeanUtils, StringUtils, DateUtils)
- `model/`: Basic data models (Result, PageResult, BaseEntity)
- `exception/`: Exception class definitions
...
**Core Components**:
- `config/`: Configuration classes (MyBatis Plus, Redis, Jackson, etc.)
...
- `aop/`: Aspect implementations (RateLimitAspect, OperationLogAspect)
...
**Main Responsibilities**:
- Define custom annotations (operation logs, data permissions, rate limiting, etc.)
- Provide business constants and enumeration classes
- Implement user context management
- Provide business utility classes

**Core Components**:
- `annotation/`: Custom annotations (`@OperationLog`, `@DataPermission`, `@RateLimit`)
- `constant/`: Business constants (error codes, permission codes, role codes)
- `enums/`: Business enumerations (LogType, MenuType, Status)
- `util/`: Business utility classes (UserContextHolder)
...
- **`effects`**: Business side-effects and logic encapsulation
  ...
  - `hooks`: Common Hooks (useTabs, useRefresh, usePagination)
...
- `access/`: Permission control implementation
