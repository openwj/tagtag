...
- `aop/`: AOP 切面实现（RateLimitAspect）
- `constant/`: 业务常量（AppMessages, CacheConstants, Permissions, Roles）
- `context/`: 用户上下文管理（AuthContext）
- `enums/`: 业务枚举（GenderEnum, StatusEnum）
- `security/`: 安全相关模型和工具（UserPrincipal, JwtClaimUtils）
