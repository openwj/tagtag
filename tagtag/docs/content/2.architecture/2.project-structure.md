...
**核心组件**：
- `utils/`: 工具类集合（BeanUtils, StringUtils, DateUtils）
- `model/`: 基础数据模型（Result, PageResult, BaseEntity）
- `exception/`: 异常类定义
- `constant/`: 常量定义
...
**核心组件**：
- `config/`: 配置类（MyBatis Plus, Redis, Jackson 等）
- `security/`: Spring Security 配置和 JWT 实现
- `web/`: 全局异常处理、CORS 配置、TraceId 过滤器
- `aop/`: 切面实现（RateLimitAspect, OperationLogAspect）
...
**主要职责**：
- 定义自定义注解（操作日志、数据权限、限流等）
- 提供业务常量和枚举类
- 实现用户上下文管理
- 提供业务工具类

**核心组件**：
- `annotation/`: 自定义注解（@OperationLog, @DataPermission, @RateLimit）
- `constant/`: 业务常量（错误码、权限码、角色码）
- `enums/`: 业务枚举（LogType, MenuType, Status）
- `util/`: 业务工具类（UserContextHolder）
